...
import { exploreRepository, type ExploreResult } from "../explorer/explore.js";

// Gold questions from docs/experimental/super-explorer/benchmarks.md — keep in sync by hand.
//
...
// See docs/experimental/planning/explorer-finetune-plan.md — "Split the gold set by
...
// docs/experimental/super-explorer/benchmarks.md. They provide a repeatable pass/fail
...
const GOLD_CHECKS: Record<string, { files: string[]; phrases: string[] }> = {
  "SE-01": { files: ["src/index.ts", "src/experimental/tools/local-explorer-task.ts"], phrases: ["experimental", "local_explorer_task", "cloud_claude_enabled", "local_worker_enabled"] },
  "SE-02": { files: ["src/index.ts", "src/experimental/workers/run-cloud-claude-task.ts", "src/experimental/workers/run-local-worker-task.ts"], phrases: ["cloud_claude_enabled", "run_cloud_claude_task", "local_worker_enabled", "run_local_worker_task"] },
  ...
  "SE-05": { files: ["src/experimental/workers/shell-allowlist.ts", "src/experimental/workers/run-local-worker-task.ts"], phrases: ["metacharacter", "tokenize", "parseallowedgitcommand", "argv", "spawnsync", "without a shell"] },
  "SE-06": { files: ["src/experimental/workers/run-cloud-claude-task.ts", "scripts/validate-cloud-bash.cjs"], phrases: ["pretooluse", "bash", "hook", "validator"] },
  "SE-07": { files: ["src/experimental/tools/local-explorer-task.ts"], phrases: ["resolvewithinroot", "rungrep", "runread", "outside"] },
  "SE-08": { files: ["src/experimental/tools/local-explorer-task.ts"], phrases: ["toolcallcount", "max_tool_calls", "set", "max_files_read"] },
  "SE-09": { files: ["src/experimental/tools/local-explorer-task.ts", "docs/experimental/benchmarks/runs/2026-09-16-local-explorer.md"], phrases: ["qwen3.5:4b", "low confidence"] },
  "SE-10": { files: ["scripts/validate-cloud-bash.cjs"], phrases: ["invalid json", "exit 0"] },
  "SE-11": { files: ["src/index.ts", "src/experimental/tools/local-explorer-task.ts"], phrases: ["5b0f7d8edecfff3fe2304d8dffc43feff4606412", "feat: add local explorer tool"] },
  "SE-12": { files: ["package.json"], phrases: ["no usable automated test", "error: no test specified", "exit 1"] },
  "SE-13": { files: ["src/experimental/explorer/discovery.ts"], phrases: ["qwen3.5:4b", "source_range", "symbol", "relationship", "adapter_fact", "git_history"] },
  "SE-14": { files: ["src/experimental/explorer/discovery.ts"], phrases: ["deliberately never returns an answer", "verification status"] },
  "SE-15": { files: ["src/experimental/explorer/discovery.ts"], phrases: ["canonical json schema", "retry"] },
  "SE-16": { files: ["src/experimental/tools/discover-evidence.ts"], phrases: ["discover_evidence", "never answers the question or verifies claims"] },
  "SE-17": { files: ["src/explorer/retrieval.ts"], phrases: ["rrf_k", "60"] },
  "SE-18": { files: ["src/explorer/retrieval.ts"], phrases: ["guard_trigger_words", "gate"] },
  "SE-19": { files: ["src/explorer/retrieval.ts"], phrases: ["score >= 4", "score >= 2"] },
  "SE-20": { files: ["src/explorer/retrieval.ts"], phrases: ["limit", "1", "100"] },
  "SE-21": { files: ["src/explorer/indexer.ts"], phrases: ["guardconditionfor", "negat", "else"] },
  "SE-22": { files: ["src/experimental/explorer/knowledge-store.ts"], phrases: ["assertcleancheckout", "clean git checkout", "commit or stash tracked changes first"] },
  "SE-23": { files: ["src/experimental/explorer/knowledge-store.ts"], phrases: ["explorer.sqlite", ".super-explorer"] },
  "SE-24": { files: ["src/experimental/explorer/knowledge-store.ts"], phrases: ["supported", "contradicted", "insufficient", "stale"] },
  "SE-25": { files: ["src/experimental/tools/save-knowledge-updates.ts", "src/experimental/explorer/knowledge-store.ts"], phrases: ["refresh_knowledge_freshness", "refreshknowledgefreshness", "saveknowledgeupdates"] },
  "SE-26": { files: ["src/explorer/structural-tools.ts"], phrases: ["id", "exact", "partial"] },
  "SE-27": { files: ["src/explorer/structural-tools.ts", "src/tools/structural-queries.ts"], phrases: ["unresolved", "excluded"] },
  "SE-28": { files: ["src/explorer/structural-tools.ts", "src/tools/structural-queries.ts"], phrases: ["findcallers", "findcallees", "resolution"] },
  "SE-29": { files: ["src/explorer/structural-tools.ts", "src/explorer/read-symbol.ts"], phrases: ["symbol id must not be empty"] },
  "SE-30": { files: ["src/explorer/outline-file.ts"], phrases: ["relativepath", "..", "sep"] },
  "SE-31": { files: ["src/tools/read-symbol.ts"], phrases: ["outline_file", "stable", "symbol id"] },
  "SE-32": { files: ["src/explorer/read-symbol.ts"], phrases: ["context_before", "context_after", "line"] },
  "SE-33": { files: ["src/explorer/outline-file.ts"], phrases: ["forward slash", "split", "join"] },
  "SE-34": { files: ["src/experimental/explorer/semantic-search.ts"], phrases: ["nomic-embed-text-v2-moe", "super_explorer_embedding_model"] },
  "SE-35": { files: ["src/experimental/explorer/semantic-search.ts"], phrases: ["commit_hash", "version", "model"] },
  "SE-36": { files: ["src/experimental/explorer/semantic-search.ts"], phrases: ["signature", "docblock", "1,600", "1600", "1_600"] },
  "SE-37": { files: ["src/experimental/explorer/synthesis.ts"], phrases: ["has no citable evidence", "supported"] },
  "SE-38": { files: ["src/experimental/explorer/verification.ts"], phrases: ["qwen3.5:4b", "strict evidence verifier"] },
  "SE-39": { files: ["src/experimental/explorer/verification.ts", "src/experimental/explorer/synthesis.ts"], phrases: ["must cite supplied evidence", "omitted_claim_ids"] },
  "SE-40": { files: ["src/experimental/explorer/verification.ts"], phrases: ["repository-relative"] },
  "SE-41": { files: ["src/experimental/explorer/synthesis.ts"], phrases: ["git:", "source_range"] },
  "SE-42": { files: ["src/experimental/explorer/framework-adapter.ts"], phrases: ["unsupported adapter index schema version", "does not match", "same commit as the generic index"] },
  "SE-43": { files: ["src/experimental/explorer/wordpress-woocommerce-adapter.ts"], phrases: ["add_action", "add_filter", "do_action", "apply_filters"] },
  "SE-44": { files: ["src/experimental/explorer/wordpress-woocommerce-adapter.ts"], phrases: ["wp_ajax_", "woocommerce_"] },
  "SE-45": { files: ["src/experimental/explorer/framework-adapter.ts", "src/experimental/explorer/wordpress-woocommerce-adapter.ts"], phrases: ["schema_version", "adapter_schema_version"] },
  "SE-46": { files: ["src/experimental/explorer/explore.ts"], phrases: ["discoverevidence", "verifyclaims", "synthesizeverifiedclaims"] },
  "SE-47": { files: ["src/experimental/explorer/explore.ts"], phrases: ["repository changed between discovery and evidence materialization"] },
  "SE-48": { files: ["src/experimental/explorer/explore.ts"], phrases: ["i could not materialize evidence for a supported answer"] },
  "SE-49": { files: ["src/experimental/explorer/git-history.ts"], phrases: ["--follow", "-l"] },
  "SE-50": { files: ["src/experimental/explorer/git-history-cli.ts"], phrases: ["blame-symbol does not accept a limit"] },
};
