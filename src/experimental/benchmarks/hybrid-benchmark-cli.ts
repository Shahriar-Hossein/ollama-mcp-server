...
import { hybridRetrieve, type RetrievalMode } from "../../explorer/retrieval.js";

const GOLD_COMMIT = "f1a75a19d1708e36f60bba0de76714acc2343492";
const DEFAULT_MODEL = "nomic-embed-text-v2-moe";

const questions = [
  ["SE-01", "Where is local_explorer_task registered, and is it enabled by default?", ["src/index.ts", "src/experimental/tools/local-explorer-task.ts"]],
  ["SE-02", "Which environment variables gate the autonomous tools, and which tool does each gate?", ["src/index.ts", "src/experimental/workers/run-cloud-claude-task.ts", "src/experimental/workers/run-local-worker-task.ts"]],
  ...
  ["SE-05", "How does the local worker prevent a model-supplied git command from using shell chaining?", ["src/experimental/workers/shell-allowlist.ts", "src/experimental/workers/run-local-worker-task.ts"]],
  ["SE-06", "What independently validates Bash commands for the cloud Claude worker, and how is it attached?", ["src/experimental/workers/run-cloud-claude-task.ts", "scripts/validate-cloud-bash.cjs"]],
  ["SE-07", "Which functions stop a local explorer model from reading outside its requested repository root?", ["src/experimental/tools/local-explorer-task.ts"]],
  ["SE-08", "How are tool-call and distinct-file-read budgets enforced in local_explorer_task?", ["src/experimental/tools/local-explorer-task.ts"]],
  ["SE-09", "What is the default model for local exploration, and what evidence justifies treating low-confidence results as untrusted?", ["src/experimental/tools/local-explorer-task.ts", "docs/experimental/benchmarks/runs/2026-09-16-local-explorer.md"]],
  ["SE-10", "If the cloud Bash validation hook receives invalid JSON, does it block the command?", ["scripts/validate-cloud-bash.cjs"]],
  ["SE-11", "Which commit introduced the local explorer tool, and which source files did that introduction add or change?", ["src/index.ts", "src/experimental/tools/local-explorer-task.ts"], "5b0f7d8edecfff3fe2304d8dffc43feff4606412"],
