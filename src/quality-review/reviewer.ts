...
export function validateReview(raw: string): Review {
  ...
  const explicitSkip = value.verdict === 'skip';
  const requestedFinding = value.verdict === 'finding' || value.verdict === 'issue' || value.verdict === 'concern';
  const finding = !explicitSkip && (requestedFinding || issues.length > 0 || (typeof value.severity === 'string' && value.severity !== 'none'));
