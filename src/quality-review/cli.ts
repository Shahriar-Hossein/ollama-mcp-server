...
async function main() {
  const {values,positionals} = parseArgs({allowPositionals:true,options:{cwd:{type:'string'},count:{type:'string'},file:{type:'string'},symbol:{type:'string'},model:{type:'string'},force:{type:'boolean'},severity:{type:'string'},help:{type:'boolean'}}});
  ...
  try {
    if (command === 'scan') print(service.scan());
      ...
      console.log(markdown(String(row.id),JSON.parse(String(row.input_json)),String(row.model),String(row.created_at),validateReview(String(row.result_json))));
    ...
    else print(await service.review({count: arg === 'next' ? 1 : count ?? (command === 'work' ? Infinity : undefined),file:values.file,symbol:values.symbol,model:values.model,force:values.force,stopped:()=>stopped,onResult:print}));
