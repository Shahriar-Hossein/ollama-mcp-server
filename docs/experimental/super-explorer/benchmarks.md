...
[`docs/experimental/benchmarks/MASTER.md`](../benchmarks/MASTER.md).
...
| SE-09 | What is the default model for local exploration, and what evidence justifies treating low-confidence results as untrusted? | `src/tools/local-explorer-task.ts`: `DEFAULT_MODEL`, `systemPrompt`, `registerLocalExplorerTask`; `docs/experimental/benchmarks/runs/2026-09-16-local-explorer.md` | The default is `qwen3.5:4b`. The prompt requires a confidence field and says low confidence instead of guessing; the public tool description directs callers to redo low-confidence searches rather than trust them. The benchmark document records the pilot limitation. |
...
Haiku row in [`docs/experimental/benchmarks/MASTER.md`](../benchmarks/MASTER.md)), answered SE-01
...
| Explorer | Questions passed | Outcome |
|---|---:|---|
| Haiku (`Explore` subagent, no Super Explorer pipeline) | 5/5 | Matches the prior `docs/experimental/benchmarks/MASTER.md` result: beats every Super Explorer configuration tried so far on this gold set. |
...
Fix applied in [`src/tools/local-explorer-task.ts`](../../../src/experimental/tools/local-explorer-task.ts):
...
`docs/experimental/planning/explorer-finetune-plan.md`'s Step 0 gold-set expansion needs.
